...
const Login = () => {
    ...
    const loginHandler = async (e) => {
        ...
        if(json.success){
            ...
            setAlert({show: true, message:'Logged-in Successfully!', type:'success'});
            navigate("/");
            setTimeout(() => {
    ...
    return (
        <div className='container mt-4'>
            ...
            <form onSubmit={loginHandler}>
                ...
                <button type="submit" role="submit" className="btn btn-primary" disabled={!(credentials.email.length!==0 && credentials.password.length>5)}>Submit</button>
            </form>
        </div>
    )
}
